package nk.rockabillyradio.views
...
class CircularSeekBar : View {
    ...
    override fun onTouchEvent(event: MotionEvent): Boolean {
        ...
        additionalRadius = if (mCircleStrokeWidth < minimumTouchTarget) { // If the width is less than the minimumTouchTarget, use the minimumTouchTarget
        ...
        additionalRadius = if (mPointerRadius < minimumTouchTarget / 2) { // If the pointer radius is less than the minimumTouchTarget, use the minimumTouchTarget
            minimumTouchTarget / 2
        } else {
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_MOVE -> if (mUserIsMovingPointer) {
                ...
                if (lockAtStart && isLockEnabled) { // TODO: Add a check if mProgress is already 0, in which case don't call the listener
                    mProgress = 0
                    recalculateAll()
                    invalidate()
                    if (mOnCircularSeekBarChangeListener != null) {
    ...
    companion object {
        ...
        protected const val DEFAULT_START_ANGLE = 0f // Geometric (clockwise, relative to 3 o'clock)
        protected const val DEFAULT_END_ANGLE = 0f // Geometric (clockwise, relative to 3 o'clock)
